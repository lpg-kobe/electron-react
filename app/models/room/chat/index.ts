...
const inititalState = immutable.fromJS({
  // 编辑器内容，用于动态修改编辑器内容，不会同步input.onchange事件，设定数组确保每次修改都会触发hooks useEffect，现暂时采用eventemitter替代
  editorValue: [''],
  // 互动区聊天列表
  list: [],
  // 互动区loading
...
export default {
  ...
  effects: {
    ...
    *updateQaaMsg({ payload }: ActionType, { call }: YieldType) {
    ...
    // 获取直播间用户列表
    *getMemberList({ payload }: ActionType, { select, put, call }: YieldType) {
      const { roomLanguage } = yield select(({ room }: any) => room.toJS())

      let { status, data: { data: members } } = yield call(getMemberList, {
        ...payload,
        headers: {
          'Accept-Language': getStore('userConfig')?.language || roomLanguage
        }
      })
      const { roomAliveUsers } = yield select((state: StateType) => state.room.toJS())

      if (status) {
        // 根据正在上麦用户初始化上麦状态
        members = members.map((mem: any) => ({
          ...mem,
          isLive: (roomAliveUsers || []).some(({ id }: any) => String(id) === String(mem.memberId))
        }))
