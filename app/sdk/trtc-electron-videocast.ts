...
export default class TrtcElectronVideocast {
    ...
    _bindEvent() {
        this.trtcInstance.on('onEnterRoom', (result: number) => {
            this.emmitter.emit(EVENT.ENTER_ROOM_SUCCESS, { result: result })
        })
        this.trtcInstance.on('onExitRoom', (result: number) => {
            this.emmitter.emit(EVENT.LEAVE_ROOM_SUCCESS, { result: result })
        })
        this.trtcInstance.on('onRemoteUserEnterRoom', (uid: string) => {
            console.log(uid);
        })
        this.trtcInstance.on('onError', (errcode: number, errmsg: string) => {
            this.emmitter.emit(EVENT.ERROR, { errcode: errcode, errmsg: errmsg })
        });
        this.trtcInstance.on('onWarning', (warningCode: number, warningMsg: string) => {
            this.emmitter.emit(EVENT.WARNING, { errcode: warningCode, errmsg: warningMsg })
        });
    ...
    _onError(event: { data: { code: any; message: any; }; }) {
        this.emmitter.emit(EVENT.ERROR, { errcode: event.data.code, errmsg: event.data.message })
    }
    ...
    _onIMMessageReceived(event: { data: any }) {
        ...
        try {
            messageData.map((item: any) => {
                if (item.conversationType === TIM.TYPES.CONV_GROUP) {// 群发消息
                    const data = JSON.parse(item.payload.data);
                    console.log(data)
                }
                if (item.conversationType === TIM.TYPES.CONV_C2C) {// 单个用户消息
                    const data = JSON.parse(item.payload.data);
                    console.log(data)
                }
            })
    ...
    */
    getScreenShareList(): [] {
        const screenList = this.trtcInstance.getScreenCaptureSources(120, 70, 20, 20);
        return screenList;
    }
